 ...
 */
class BuilderChainExtractor implements SpoonPatternExtractor {
    ...
    /** Returns true when {@code expr} is a builder terminal — outermost call is {@code .build()}. */
    ...
    /**
     * Walks inward through the chain to find the {@code builder()} call and returns
     * the class name it was invoked on (e.g., {@code Address.builder()} → {@code "Address"}).
     * Returns a simple name; FQN qualification is deferred to FieldRefQualifier.
     */
    private String extractBuilderTargetClass(CtExpression<?> expr) {
        ...
        // Last resort: return type of the build() call
    ...
    /**
     * Recursively walks the builder chain, emitting one FieldMapping per field-setter call.
     */
