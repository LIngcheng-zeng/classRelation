 ...
 *   → User.phone → Account.fullMobile  (if constructor declares {@code String fullMobile})
 *   → User.id    → Account.userId
 */
class ConstructorCallExtractor implements SpoonPatternExtractor {
            ...
            try {
                ...
                if (ctorDecl != null) {
                    ...
                    for (int i = 0; i < limit; i++) {
                        emitIfValid(helper.extractSourceSide(args.get(i), aliasMap),
                                    sinkClass, params.get(i).getSimpleName(),
                                    ctorCall.toString(), location, results, helper);
                    }
                } else {
                    // Positional fallback when declaration is unavailable
